...
# COCO classes
CLASSES =  ['ba', 'eo', 'erb', 'ig', 'lym', 'mono', 'neut', 'platelet']
...
class AttentionVisualizer:
    ...
    def enc_attention_visualizaion(self, path):
        ...
        if all_inspect == True:
            plt.savefig('output/visualization_attention/cell_all_points_enc_query_' + str(query_id))
        else:
            if level_inspect == True : 
    ...
    def dec_attention_visualizaion(self, path):
        ...
        plt.scatter(reference_point_rescale[:, 0], reference_point_rescale[:, 1], s=400, c='white', marker='X')
        if all_inspect == True:
            plt.savefig('output/visualization_attention/cell_all_points_dec_query_' + str(query_id))
        else:
            if level_inspect == True : 
                #plt.savefig('output/visualization_dec_attention/query_' + str(query_id) + '_level_' + str(level_index) + '_heads_all_points_all.jpg')
                plt.savefig('output/visualization_attention/cell_casade_6_dec_query_' + str(query_id))
            else:
...
class output_visualizer:
    ...
    def standard_visualization(self):
        ...
        # Configure axis
        ax = plt.gca()
        label_list = ['ba', 'eo', 'erb', 'ig', 'lym', 'mono', 'neut', 'platelet']
        CLASSES = ['ba', 'eo', 'erb', 'ig', 'lym', 'mono', 'neut', 'platelet']
